...
- **Verified:** A credential-backed smoke request through `/api/generate` returned `Pong.` using
  `openai-gpt-4.1`; the provider branch was merged in PR #14. Server (22) and client (118) tests,
  plus Server, Firebase Functions, and Client builds passed.
- **In flight:** —.
- **Next steps:** Keep real keys in backend configuration only; do not commit `.env` files.
